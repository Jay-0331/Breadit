...
const Post: FC<PostProps> = ({ subredditName, post, commentAmt, votesAmt, currentVote }) => {
    ...
    return <div className='rounded-md bg-white dark:bg-black shadow overflow-hidden'>
        <div className='px-6 py-4 flex justify-between'>
            {/* TODO: PostVotes */}
            <PostVoteClient postId={post.id} initialVote={currentVote?.type} initialVoteAmt={votesAmt}/>

            <div className='w-0 flex-1'>
                <div className='max-h-40 mt-1 text-xs text-gray-600 dark:text-gray-400'>
                    {subredditName ? (
                        <>
                            <a 
                                className='underline text-zinc-900 dark:text-gray-100 text-sm underline-offset-2' 
